...
/**
 * bank enrolment external functions.
 *
 * @package   enrol_bank
 ...
 * @since     Moodle 2.6
 */
class enrol_bank_external extends external_api {
    ...
    public static function get_instance_info_parameters() {
        return new external_function_parameters(
                array('instanceid' => new external_value(PARAM_INT, 'instance id of bank enrolment plugin.'))
            );
    }

    /**
     * Return bank-enrolment instance information.
     *
     * @param int $instanceid instance id of bank enrolment plugin.
    ...
    public static function get_instance_info($instanceid) {
        ...
        $params = bank::validate_parameters(bank::get_instance_info_parameters(), array('instanceid' => $instanceid));

        // Retrieve bank enrolment plugin.
        $enrolplugin = enrol_get_plugin('bank');
        if (empty($enrolplugin)) {
            throw new moodle_exception('invaliddata', 'error');
        }

        bank::validate_context(context_system::instance());
    ...
    public static function enrol_user_parameters() {
        return new external_function_parameters(
            array(
                ...
                'instanceid' => new external_value(PARAM_INT, 'Instance id of bank enrolment plugin.', VALUE_DEFAULT, 0)
            )
        );
    }

    /**
     * bank enrol the current user in the given course.
     *
    ...
    public static function enrol_user($courseid, $password = '', $instanceid = 0) {
        ...
        $params = bank::validate_parameters(bank::enrol_user_parameters(),
                                            array(
                                                'courseid' => $courseid,
                                                'password' => $password,
        ...
        // Retrieve the bank enrolment plugin.
        $enrol = enrol_get_plugin('bank');
        if (empty($enrol)) {
            throw new moodle_exception('canntenrol', 'enrol_bank');
        }

        // We can expect multiple bank-enrolment instances.
        $instances = array();
        $enrolinstances = enrol_get_instances($course->id, true);
        foreach ($enrolinstances as $courseenrolinstance) {
            if ($courseenrolinstance->enrol == "bank") {
                // Instance specified.
        ...
        foreach ($instances as $instance) {
            $enrolstatus = $enrol->can_bank_enrol($instance);
            if ($enrolstatus === true) {
                if ($instance->password and $params['password'] !== $instance->password) {
                    ...
                    if ($instance->customint1) {
                        ...
                        if (!enrol_bank_check_group_enrolment_key($course->id, $params['password'])) {
                            $warnings[] = array(
                                'item' => 'instance',
                                'itemid' => $instance->id,
                                'warningcode' => '2',
                                'message' => get_string('passwordinvalid', 'enrol_bank')
                            );
                            continue;
                        }
                    } else {
                        if ($enrol->get_config('showhint')) {
                            ...
                            $warnings[] = array(
                                ...
                                'itemid' => $instance->id,
                                'warningcode' => '3',
                                'message' => s(get_string('passwordinvalidhint', 'enrol_bank', $hint)) // message is PARAM_TEXT.
                            );
                            continue;
                        } else {
                            $warnings[] = array(
                ...
                $enrol->enrol_bank($instance, (object) $data);
                $enrolled = true;
                break;
            } else {
                $warnings[] = array(

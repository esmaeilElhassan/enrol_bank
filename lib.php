<?php
class enrol_bank_plugin extends enrol_plugin {
    ...
    public function get_info_icons(array $instances) {
        ...
        foreach ($instances as $instance) {
            if ($this->can_bank_enrol($instance, false) !== true) {
                // User can not enrol himbank.
                ...
                // would hide bank-enrolment icons from guests.
                continue;
            }
        ...
        $icons = array();
        if ($nokey) {
            $icons[] = new pix_icon('withoutkey', get_string('pluginname', 'enrol_bank'), 'enrol_bank');
        }
        if ($key) {
            $icons[] = new pix_icon('withkey', get_string('pluginname', 'enrol_bank'), 'enrol_bank');
        }
        return $icons;
    }
    ...
    public function show_enrolme_link(stdClass $instance) {
        ...
        if (true !== $this->can_bank_enrol($instance, false)) {
            return false;
        }
    ...
    public function can_add_instance($courseid) {
        ...
        if (!has_capability('moodle/course:enrolconfig', $context) or !has_capability('enrol/bank:config', $context)) {
            return false;
        }
    ...
    public function enrol_page_hook(stdClass $instance) {
        ...
        $enrolstatus = $this->can_bank_enrol($instance);

        if (true === $enrolstatus) {
            // This user can bank enrol using this instance.
            $form = new enrol_bank_enrol_form(null, $instance);
            $instanceid = optional_param('instance', 0, PARAM_INT);
            if ($instance->id == $instanceid) {
                if ($data = $form->get_data()) {
                    $this->enrol_bank($instance, $data);
                }
            }
        } else {
            // This user can not bank enrol using this instance. Using an empty form to keep
            // the UI consistent with other enrolment plugins that returns a form.
            $data = new stdClass();
            $data->header = $this->get_instance_name($instance);
            $data->info = $enrolstatus;

            // The can_bank_enrol call returns a button to the login page if the user is a
            // guest, setting the login url to the form if that is the case.
            $url = isguestuser() ? get_login_url() : null;
            $form = new enrol_bank_empty_form($url, $data);
        }
     ...
     */
    public function can_bank_enrol(stdClass $instance, $checkuserenrolment = true) {
        ...
        if ($checkuserenrolment) {
            ...
            if ($DB->get_record('user_enrolments', array('userid' => $USER->id, 'enrolid' => $instance->id))) {
                return get_string('canntenrol', 'enrol_bank');
            }
        }
        ...
        if ($instance->enrolstartdate != 0 and $instance->enrolstartdate > time()) {
            return get_string('canntenrolearly', 'enrol_bank', userdate($instance->enrolstartdate));
        }

        if ($instance->enrolenddate != 0 and $instance->enrolenddate < time()) {
            return get_string('canntenrollate', 'enrol_bank', userdate($instance->enrolenddate));
        }
        ...
        if ($instance->customint3 > 0) {
            ...
            if ($count >= $instance->customint3) {
                // Bad luck, no more bank enrolments here.
                return get_string('maxenrolledreached', 'enrol_bank');
            }
        }
        ...
        if ($instance->customint5) {
            ...
            if (!cohort_is_member($instance->customint5, $USER->id)) {
                ...
                return markdown_to_html(get_string('cohortnonmemberinfo', 'enrol_bank', $a));
            }
        }
    ...
    public function get_enrol_info(stdClass $instance) {
        ...
        $instanceinfo->status = $this->can_bank_enrol($instance);

        if ($instance->password) {
            $instanceinfo->requiredparam = new stdClass();
            $instanceinfo->requiredparam->enrolpassword = get_string('password', 'enrol_bank');
        }
        ...
        if ((true === $instanceinfo->status) && $instance->password) {
            $instanceinfo->wsfunction = 'enrol_bank_get_instance_info';
        }
        return $instanceinfo;
    }
    ...
    protected function email_welcome_message($instance, $user) {
        ...
        if (trim($instance->customtext1) !== '') {
            ...
            $messagetext = get_string('welcometocoursetext', 'enrol_bank', $a);
            $messagehtml = text_to_html($messagetext, null, false, true);
        }

        $subject = get_string('welcometocourse', 'enrol_bank', format_string($course->fullname, true, array('context'=>$context)));
    ...
    public function sync(progress_trace $trace, $courseid = null) {
        ...
        $trace->output('Verifying bank-enrolments...');
        ...
        // Note: the logic of bank enrolment guarantees that user logged in at least once (=== u.lastaccess set)
        ...
        $sql = "SELECT e.*, ue.userid
                  ...
                  JOIN {enrol} e ON (e.id = ue.enrolid AND e.enrol = 'bank' AND e.customint2 > 0)
        ...
        $trace->output('...user bank-enrolment updates finished.');
        $trace->finished();
    ...
    /**
     * Returns the user who is responsible for bank enrolments in given instance.
     *
     * Usually it is the first editing teacher - the person with "highest authority"
     * as defined by sort_by_roleassignment_authority() having 'enrol/bank:manage'
     * capability.
     *
    ...
    protected function get_enroller($instanceid) {
        ...
        if ($users = get_enrolled_users($context, 'enrol/bank:manage')) {
        ...
        if ($merge and $instances = $DB->get_records('enrol', $merge, 'id')) {
            ...
            if (!empty($data->customint5)) {
                if ($step->get_task()->is_samesite()) {
                    ...
                    // Use some id that can not exist in order to prevent bank enrolment,
    ...
    public function restore_role_assignment($instance, $roleid, $userid, $contextid) {
        ...
        // we do not use component in manual or bank enrol.
    ...
    public function can_delete_instance($instance) {
        $context = context_course::instance($instance->courseid);
        return has_capability('enrol/bank:config', $context);
    }
    ...
    public function can_hide_show_instance($instance) {
        ...
        if (!has_capability('enrol/bank:config', $context)) {
            return false;
        }
    ...
    public function edit_instance_form($instance, MoodleQuickForm $mform, $context) {
        ...
        $mform->addElement('select', 'status', get_string('status', 'enrol_bank'), $options);
        $mform->addHelpButton('status', 'status', 'enrol_bank');

        $options = $this->get_newenrols_options();
        $mform->addElement('select', 'customint6', get_string('newenrols', 'enrol_bank'), $options);
        $mform->addHelpButton('customint6', 'newenrols', 'enrol_bank');
        $mform->disabledIf('customint6', 'status', 'eq', ENROL_INSTANCE_DISABLED);

        $passattribs = array('size' => '20', 'maxlength' => '50');
        $mform->addElement('passwordunmask', 'password', get_string('password', 'enrol_bank'), $passattribs);
        $mform->addHelpButton('password', 'password', 'enrol_bank');
        ...
        $mform->addElement('select', 'customint1', get_string('groupkey', 'enrol_bank'), $options);
        $mform->addHelpButton('customint1', 'groupkey', 'enrol_bank');

        $roles = $this->extend_assignable_roles($context, $instance->roleid);
        $mform->addElement('select', 'roleid', get_string('role', 'enrol_bank'), $roles);

        $options = array('optional' => true, 'defaultunit' => 86400);
        $mform->addElement('duration', 'enrolperiod', get_string('enrolperiod', 'enrol_bank'), $options);
        $mform->addHelpButton('enrolperiod', 'enrolperiod', 'enrol_bank');
        ...
        $mform->addElement('date_time_selector', 'enrolstartdate', get_string('enrolstartdate', 'enrol_bank'), $options);
        $mform->setDefault('enrolstartdate', 0);
        $mform->addHelpButton('enrolstartdate', 'enrolstartdate', 'enrol_bank');

        $options = array('optional' => true);
        $mform->addElement('date_time_selector', 'enrolenddate', get_string('enrolenddate', 'enrol_bank'), $options);
        $mform->setDefault('enrolenddate', 0);
        $mform->addHelpButton('enrolenddate', 'enrolenddate', 'enrol_bank');

        $options = $this->get_longtimenosee_options();
        $mform->addElement('select', 'customint2', get_string('longtimenosee', 'enrol_bank'), $options);
        $mform->addHelpButton('customint2', 'longtimenosee', 'enrol_bank');

        $mform->addElement('text', 'customint3', get_string('maxenrolled', 'enrol_bank'));
        $mform->addHelpButton('customint3', 'maxenrolled', 'enrol_bank');
        ...
        if (count($cohorts) > 1) {
            $mform->addElement('select', 'customint5', get_string('cohortonly', 'enrol_bank'), $cohorts);
            $mform->addHelpButton('customint5', 'cohortonly', 'enrol_bank');
        } else {
        ...
        $mform->addElement('select', 'customint4', get_string('sendcoursewelcomemessage', 'enrol_bank'),
                enrol_send_welcome_email_options());
        $mform->addHelpButton('customint4', 'sendcoursewelcomemessage', 'enrol_bank');

        $options = array('cols' => '60', 'rows' => '8');
        $mform->addElement('textarea', 'customtext1', get_string('customwelcomemessage', 'enrol_bank'), $options);
        $mform->addHelpButton('customtext1', 'customwelcomemessage', 'enrol_bank');

        if (enrol_accessing_via_instance($instance)) {
            $warntext = get_string('instanceeditbankwarningtext', 'core_enrol');
            $mform->addElement('static', 'bankwarn', get_string('instanceeditbankwarning', 'core_enrol'), $warntext);
        }
    }
    ...
    public function edit_instance_validation($data, $files, $instance, $context) {
        ...
        if ($data['status'] == ENROL_INSTANCE_ENABLED) {
            if (!empty($data['enrolenddate']) and $data['enrolenddate'] < $data['enrolstartdate']) {
                $errors['enrolenddate'] = get_string('enrolenddaterror', 'enrol_bank');
            }
        }
    ...
    /**
     * Gets a list of roles that this user can assign for the course as the default for bank-enrolment.
     *
     * @param context $context the context.
     * @param integer $defaultrole the id of the role that is set as the default for bank-enrolment
    ...
    public function get_welcome_email_contact($sendoption, $context) {
        ...
        if ($sendoption == ENROL_SEND_EMAIL_FROM_COURSE_CONTACT) {
            ...
            // Send as the first user with enrol/bank:holdkey capability assigned in the course.
            list($sort) = users_order_by_sql('u');
            $keyholders = get_users_by_capability($context, 'enrol/bank:holdkey', 'u.*', $sort);
            if (!empty($keyholders)) {
 ...
 */
function enrol_bank_get_fontawesome_icon_map() {
    return [
        'enrol_bank:withkey' => 'fa-key',
        'enrol_bank:withoutkey' => 'fa-sign-in',
    ];
}
